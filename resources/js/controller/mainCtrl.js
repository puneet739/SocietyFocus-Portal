...
function($rootScope, $scope, $state, $translate, $localStorage, $window, $document, $timeout, cfpLoadingBar) {
	...
	/* Internet disconnection message*/
	 ...
	 $rootScope.$watch('online', function(newStatus) {
		if(newStatus == true){
			if($rootScope.growlId != 0){
				growl.success("Your device is connected to the internet. You are Good To Go.", {ttl: 2000});
				$rootScope.growlId.destroy();
			}
		}else{
			$rootScope.growlId = growl.error("Your device lost its internet connection. Changes will not be saved.", {ttl: -1, disableCloseButton: true});
		}
